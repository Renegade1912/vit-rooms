    ...
    <Dialog :open="isOpen" @update:open="handleOpen">
        <DialogContent @interact-outside="(event) => event.preventDefault()">
            ...
            <DialogHeader>
                ...
                <DialogDescription>
                    <div>
                        Wollen Sie den Tag <b class="text-primary">{{ tag?.desc }} ({{ tag?.mac }})</b> wirklich
                        löschen?
                    </div>
...
import { useDeleteDialog } from '~/components/Tags/columns';
import { useToast } from '@/components/ui/toast/use-toast';
import { useFocus } from '@vueuse/core';
const { isOpen, close, tag } = useDeleteDialog();
    ...
    onResponse({ response }) {
        toast({
            ...
            description: response.ok
                ? h('span', null, [
                      'Der Tag ',
                      h('b', null, `${tag.value?.desc} (${tag.value?.mac})`),
